...
def ReadingExcel (path, Longitud, Latitud):
    ...
    d=datetime.today()
    mes=d.month
    #mes = int(input("Introduzca el mes a consultar en formato numerico para la radiacion extraterrestre: "))
    #HEMISFERIO NORTE
    NHemis = True
    if Latitud < 0:
    ...
    if True!=isEven:
        if(LatitudOriginal-Latitud > 0):
            ...
            Latitud=Latitud-1
    #print("Latitud formateada " , Latitud)
    ...
    for row in ws.iter_rows(min_row=ActualRow, max_col=mes, min_col=mes, max_row=ActualRow, values_only=True):
        ...
        Ra=result
    return Ra
Ra=ReadingExcel('/home/pi/Desktop/8-GIT/repos/documentation/19-Ra_zonas.xlsx', Lon, Lat)
